...
from IN5 import def_instrument
...
def data_vanadium():
    return get_NXdetector_data(
        os.path.abspath("institutes/ILL/instruments/IN5/HEAD/mcstas/data/199804.nxs")
    )
...
def test_name(set_instr):
    myinstrument = set_instr
    assert myinstrument.name == "IN5"


def test_detector_size(data_vanadium):
    detectors = data_vanadium[0]
    assert detectors["detector"].shape == (384, 256, 512)
...
def test_BCs(config0, tmp_path):
    ...
    *_, calc = myinstrument.calculators
    ...
    for component in reversed(mycalc.component_list):
        if component.name == "Chopper6":
            break
        mycalc.remove_component(component)

    monitor = mycalc.add_component(
        "monitor", "Monitor_nD", AT=2.001, RELATIVE="Chopper6"
    )
...
def test_BC1(config0, tmp_path):
    ...
    myinstrument = config0
    myinstrument.calculators["OriginCalc"].parameters["rpm"].add_option(0, True)
    myinstrument.master["rpm"] = 0
    myinstrument.settings(ncount=1e5)
    ...
    *_, calc = myinstrument.calculators
    ...
    for component in reversed(mycalc.component_list):
        if component.name == "Chopper1":
            break
        mycalc.remove_component(component)

    monitor = mycalc.add_component(
        "monitor", "Monitor_nD", AT=0.01, RELATIVE="Chopper1"
    )
    monitor.set_parameters(
        xwidth=0.150,
        yheight=0.500,
    ...
    intensity = []
    phases = [20, 29, 30, 31]
    phases = [10]  # range(10, 15, 5)
    for llambda in [2, 4, 6]:
        myinstrument.master["lambda"] = llambda * ureg.angstrom

        for phase in phases:
            bc = mycalc.get_component("Chopper1")
            bc.phase = phase
            bc.nu = 1
            bc.verbose = 1
            ...
            fig.savefig(
                os.path.join(
                    tmp_path, "diagnostics_bc1_ph{}_e{}.pdf".format(phase, llambda)
                )
            )
